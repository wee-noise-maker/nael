...
int nael_rtaudio_init (unsigned int sample_rate, unsigned int frames);

int nael_rtaudio_cleanup (void);

extern void nael_rtaudio_callback (void *outbuf, unsigned int nFrames);
...
static int
nael_rtaudio_callback_internal(
	void			*outbuf,
	void			*inbuf,
	...
	void			*userdata)
{
  ...
  nael_rtaudio_callback(outbuf, nFrames);

  return 0;
}

int
nael_rtaudio_init (unsigned int sample_rate, unsigned int frames)
{
  RtAudio *audio = NULL;
  ...
  try
    {
      audio->openStream (outParam, NULL, RTAUDIO_SINT16, sample_rate, &bufsize,
                         nael_rtaudio_callback_internal, NULL);
